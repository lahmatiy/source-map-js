...
import { encode } from './base64-vlq.js';
import { MappingList } from './mapping-list.js';
import { getArg, relative, join, compareByGeneratedPositionsInflated } from './util.js';
...
export class SourceMapGenerator {
  constructor(aArgs) {
    ...
    this._version = 3;
    this._file = getArg(aArgs, 'file', null);
    this._sourceRoot = getArg(aArgs, 'sourceRoot', null);
    this._skipValidation = getArg(aArgs, 'skipValidation', false);
  ...
  static fromSourceMap(sourceMapConsumer) {
    ...
    sourceMapConsumer.sources.forEach((sourceFile) => {
      ...
      if (sourceRoot !== null) {
        sourceRelative = relative(sourceRoot, sourceFile);
      }
   ...
   */
  addMapping(args) {
    const generated = getArg(args, 'generated');
    const original = getArg(args, 'original', null);
    let source = getArg(args, 'source', null);
    let name = getArg(args, 'name', null);
  ...
  setSourceContent(sourceFile, sourceContent) {
    if (this._sourceRoot != null) {
      sourceFile = relative(this._sourceRoot, sourceFile);
    }
  ...
  applySourceMap(sourceMapConsumer, sourceFile, sourceMapPath) {
      ...
      if (mapping.source === sourceFile && mapping.originalLine !== null) {
        ...
        if (original.source !== null) {
          ...
          if (sourceMapPath != null) {
            mapping.source = join(sourceMapPath, mapping.source)
          }
          if (sourceRoot != null) {
            mapping.source = relative(sourceRoot, mapping.source);
          }
    ...
    sourceMapConsumer.sources.forEach((sourceFile) => {
      ...
      if (content != null) {
        if (sourceMapPath != null) {
          sourceFile = join(sourceMapPath, sourceFile);
        }
  ...
  _serializeMappings() {
    ...
    for (let i = 0; i < mappings.length; i++) {
      ...
      if (mapping.source != null) {
        ...
        next += encode(sourceIdx - previousSource);
        ...
        if (mapping.name != null) {
          ...
          next += encode(nameIdx - previousName);
