...
class SourceMapConsumer {
  constructor(sourceMap, aSourceMapURL) {
    ...
    if (typeof sourceMap === 'string') {
      sourceMap = util.parseSourceMapInput(sourceMap);
    }
 ...
 */
  ...
  allGeneratedPositionsFor(args) {
    const line = util.getArg(args, 'line');
    ...
    const needle = {
      source: util.getArg(args, 'source'),
      originalLine: line,
      originalColumn: util.getArg(args, 'column', 0)
    };
...
class BasicSourceMapConsumer extends SourceMapConsumer {
  constructor(sourceMap, sourceMapURL) {
    ...
    this._absoluteSources = this._sources.toArray().map((source) =>
      util.computeSourceURL(sourceRoot, source, sourceMapURL)
    );
  ...
  get consumer() { return SourceMapConsumer; }
   ...
   * @param {String} aSourceMapURL The URL at which the source map can be found (optional)
  ...
  static fromSourceMap(aSourceMap, aSourceMapURL) {
    ...
    smc._absoluteSources = smc._sources.toArray().map((source) =>
      util.computeSourceURL(smc.sourceRoot, source, aSourceMapURL)
    );
  ...
  _parseMappings(aStr) {
    ...
    for (const mappings of originalMappings) {
      if (mappings != null) {
        quickSort(mappings, util.compareByOriginalPositionsNoSource);
      }
    }
  ...
  _findMapping(needle, mappings, lineName, columnName, comparator, bias) {
    ...
    return binarySearch.search(needle, mappings, comparator, bias);
  }
  ...
  originalPositionFor(args) {
    ...
    const index = this._findMapping(
      ...
      util.getArg(args, 'bias', SourceMapConsumer.GREATEST_LOWER_BOUND)
    );
  ...
  generatedPositionFor(args) {
    let source = this._findSourceIndex(util.getArg(args, 'source'));
    ...
    const needle = {
      source,
      originalLine: util.getArg(args, 'line'),
      originalColumn: util.getArg(args, 'column')
    };
...
class IndexedSourceMapConsumer extends SourceMapConsumer {
  constructor(sourceMap, sourceMapURL) {
    ...
    this._sections = sections.map((section) => {
      ...
      const offset = util.getArg(section, 'offset');
      ...
      return {
        ...
        consumer: new SourceMapConsumer(util.getArg(section, 'map'), sourceMapURL)
      }
    });
  }
  ...
  originalPositionFor(aArgs) {
    ...
    const sectionIndex = binarySearch.search(needle, this._sections, (needle, section) =>
   ...
   */
  generatedPositionFor(args) {
    const source = util.getArg(args, 'source');
